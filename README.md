...
The official Claude Code installer (`npm install -g @anthropic-ai/claude-code`) and the official Homebrew cask both download from `downloads.claude.ai`, which is blocked on the IH VPN. This tap provides VPN-safe mirror casks that download from GitHub releases instead.
