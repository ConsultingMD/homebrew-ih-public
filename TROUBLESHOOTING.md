...
**Cause:** PR #150 (pending) adds a CODEOWNERS rule requiring at least one reviewer for cask changes. Until that PR is merged and the auto-merge configuration accounts for it, the workflow cannot fully self-service.

**Fix:** Request a review from a member of the `@ConsultingMD/homebrew-ih-public` CODEOWNERS group and approve the PR manually. This is expected behavior until the auto-merge + CODEOWNERS integration is resolved in #150.
